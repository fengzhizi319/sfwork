...
> 
> 本计划采用**顺序推进、逐步深入**的策略：项目周期为 **2025 年 6 月 15 日至 11 月 30 日**，先通过平台深度调研跑通完整链路，再依次开发本地差分隐私、数据安全分类分级，最终沉淀对现有联邦学习 / 安全多方计算平台的熟练使用。
...
| 顺序 | 阶段 | 目标 | 周期 / 时间 |
|---|---|---|---|
| 1 | 平台深度调研与 Privacy 模块跑通 | 充分理解 SecretFlow 全栈架构（前端 / 后端 / Kuscia / 算法），并以简单 **Privacy** 组件为例跑通二次开发完整流程 | 6.15 – 7.15 |
| 2 | 本地差分隐私模块开发 | 设计并实现 Local DP 算法，完成组件化、平台集成、镜像构建与端到端验证 | 7.15 – 9.15 |
| 3 | 数据安全分类分级 | 设计并实现基于规则 + NER + LLM 的分类分级系统，集成到 SecretPad 治理体系 | 9.15 – 10.30 |
| 4 | 精通联邦学习 / MPC 平台 | 沉淀 SOP、流水线模板、培训材料与实战 Lab，形成平台熟练使用与交付能力 | 10.30 – 11.30 |
...
```mermaid
gantt
    title sfwork 二次开发项目排期（2025.6.15 – 2025.11.30）
    dateFormat  YYYY-MM-DD
    axisFormat  %m/%d

    section Phase 1
    平台深度调研与 Privacy 模块跑通    :p1, 2025-06-15, 31d
    ...
    分类分级引擎与平台集成开发          :p3dev, after p3d, 28d
    分类分级测试与验收                  :p3t, after p3dev, 10d

    section Phase 4
    平台熟练使用 / SOP / 培训交付       :p4, 2025-10-30, 31d
```
...
## 3. Phase 1：平台深度调研与 Privacy 模块跑通（6.15 – 7.15）
...
## 4. Phase 2：本地差分隐私模块开发（7.15 – 9.15）
...
| 序号 | 任务 | 交付物 | 验收标准 |
|---|---|---|---|
| 2.1.1 | Local DP 算法设计 | 算法设计文档 | 含敏感度分析、噪声公式、预算管理策略 |
| 2.1.2 | 平台集成方案设计 | 集成设计文档 | 明确组件参数、输入输出、SecretPad API 变更 |
| 2.1.3 | 前端交互设计 | 原型 | DP 参数配置、预览、报告展示 |
...
## 5. Phase 3：数据安全分类分级（9.15 – 10.30）

### 5.1 阶段目标

- 设计完善《分类分级算法设计》，实现 L1–L5 五级分类分级系统；
...
| 子阶段 | 周期 | 重点 |
|---|---|---|
| 3.1 系统设计 | 2 周 | 五级矩阵、三层引擎、Data Catalog 模型、策略路由 |
| 3.2 引擎与平台开发 | 3 周 | Rule/NER/LLM 实现、Catalog/Policy API、前端页面 |
...
| 序号 | 任务 | 交付物 | 验收标准 |
|---|---|---|---|
| 3.1.1 | 五级分类矩阵细化 | 分类分级标准 V1.0 | 覆盖医疗/金融/通用 PII |
| 3.1.2 | 三层引擎接口设计 | 接口文档 | 明确 `RuleEngineManager` / `SmallNEREngine` / `LLMClassifier` |
| 3.1.3 | Data Catalog 数据模型设计 | ER 图 | 含 Asset / SecurityTag / LineageEdge / ReviewTicket |
| 3.1.4 | 前端页面与审批流程设计 | 原型 | 资产目录、标签、复核、策略配置 |
...
## 6. Phase 4：开发联邦学习 / 安全多方计算平台（10.30 – 11.30）
...
| 周次 | 任务 | 交付物 | 验收标准 |
|---|---|---|---|
| W1 | 梳理平台核心操作流程 | SOP 初稿 | 覆盖节点/数据/任务/日志 |
| W2 | 开发流水线模板 | 6+ 模板 | 每个模板可一键导入运行 |
| W3 | 制作培训材料与 Lab | PPT/视频/Lab | 通过内部试讲 |
| W4 | 培训交付与测评 | 测评报告 | 参训人员 80% 通过测评 |
...
```mermaid
gantt
    title 总体里程碑（2025.6.15 – 2025.11.30）
    dateFormat  YYYY-MM-DD
    axisFormat  %m/%d

    section Phase 1
    平台调研与 Privacy 跑通        :m1, 2025-06-15, 31d
    ...
    分类分级开发                    :m3dev, after m3d, 28d
    分类分级测试验收                :m3t, after m3dev, 10d

    section Phase 4
    平台精通与培训交付              :m4, 2025-10-30, 31d
```

| 里程碑 | 时间 | 关键交付 |
|---|---|---|
| M1：平台跑通 | 7.15 | 完整二次开发链路跑通、《sfwork 二次开发入门手册》 |
| M2：Local DP 设计完成 | 7.29 | 算法设计文档、集成方案、原型 |
| M3：Local DP 可用 | 9.02 | 组件注册、镜像构建、前端可用 |
| M4：Local DP 验收通过 | 9.15 | 测试报告、《本地差分隐私算法设计与集成手册》 |
| M5：分类分级设计完成 | 9.22 | 五级矩阵、接口/数据模型设计 |
| M6：分类分级可用 | 10.20 | 三层引擎 + Catalog + 前端可用 |
| M7：分类分级验收通过 | 10.30 | 端到端场景通过、《数据安全分类分级系统设计与集成手册》 |
| M8：平台精通交付 | 11.30 | SOP、模板库、培训材料、测评报告 |
...
| 角色 | 主要职责 |
|---|---|
| 项目经理/产品经理 | 排期、验收、培训组织、SOP 沉淀 |
| 隐私计算算法工程师 | SecretFlow 组件、Local DP、分类分级引擎、模型选型 |
| Java 后端工程师 | SecretPad 扩展、Data Catalog、Policy API、Kuscia 集成 |
| 前端工程师 | 组件面板、流水线模板、资产目录、复核/策略页面 |
| DevOps/测试工程师 | 镜像构建、Kuscia 部署、测试、性能压测 |
| 安全顾问 | 五级分类标准、隐私预算策略、合规审计 |
...
| 风险 | 影响 | 应对措施 |
|---|---|---|
| Phase 1 学习周期被拉长 | 高 | 明确每日学习目标，以 Privacy 组件跑通为硬截止 |
