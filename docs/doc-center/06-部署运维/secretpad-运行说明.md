...
> 适用范围：SecretPad 源码仓库（`/home/charles/code/secretpad`），前端源码位于同一仓库的 `frontend-src/` 目录下。
...
如果你不想在系统全局安装 JDK/Maven/Node，可以下载到项目 `.tools` 目录：
...
**重要**：SecretPad 后端**不需要也不应该**直接连接 SecretFlow 容器，原因如下：

1. **SecretFlow 不是常驻服务**：SecretFlow 以 AppImage 形式注册到 Kuscia，仅在任务执行时由 Kuscia 动态启动，任务完成后自动销毁
2. **架构设计**：SecretPad → Kuscia → SecretFlow 的分层架构，SecretPad 只与 Kuscia 通信
3. **自动管理**：Kuscia 负责任务调度、SecretFlow 容器启停、配置注入等所有底层细节
...
只要正确配置了 Kuscia 连接，SecretFlow 就会自动可用，无需额外配置：
...
# ❌ 错误的做法（不需要配置 SecretFlow 地址）
# export SECRETFLOW_ADDRESS=xxx  # 不存在此配置项
# export SECRETFLOW_PORT=xxx     # 不需要手动指定
```
...
# 应看到 sf-serving-image 和 scql-image
```

2. **提交测试任务**：
   - 登录 SecretPad 前端（http://localhost:8000）
   - 创建项目并编排任务流
   - 运行任务，观察任务状态
   - 如果 SecretFlow 正常，任务会从 `Pending` → `Running` → `Succeeded`

3. **查看 SecretFlow 容器日志**（任务运行时）：
```bash
# 查看 alice 节点上的 SecretFlow 容器
...
如果你只需要验证 SecretPad 的基础功能（不涉及隐私计算任务执行），可以不部署完整的 SecretFlow 环境：
...
- ✅ 项目管理和任务编排（DAG 编辑器）
- ❌ 任务执行（需要 SecretFlow）
- ❌ 模型训练和预测（需要 SecretFlow）

**最小化配置**：

即使不执行任务，仍需保证 Kuscia 正常运行（因为节点管理、数据管理等功能依赖 Kuscia DataMesh）：
...
|---------|---------|------|
| 用户登录 | ✅ | 正常 |
| 节点管理 | ✅ | 可查看 alice/bob 节点状态 |
| 数据管理 | ✅ | 可注册和查看数据源 |
| 项目管理 | ✅ | 可创建项目和邀请参与方 |
| 任务编排 | ✅ | 可使用 DAG 编辑器拖拽组件 |
| 任务保存 | ✅ | 可保存任务流定义 |
| **任务执行** | ❌ | 点击运行会失败（缺少 SecretFlow） |
| 模型管理 | ⚠️ | 可查看但无法训练新模型 |
| 任务日志 | ❌ | 无任务执行，无日志 |

**如果需要完整功能**：

确保 `install-kuscia-only.sh` 部署时会自动注册 SecretFlow AppImage，无需额外操作。脚本内部会执行：

```bash
# install-kuscia-only.sh 内部自动执行（无需手动操作）
applySfServingAppImage  # 注册 SecretFlow Serving AppImage
applySfScqlAppImage     # 注册 SCQL AppImage（如果启用）
```
...
如果你希望在 macOS 上运行 SecretPad 前端与后端，且**不需要部署 Kuscia 容器**，可以使用本仓库 `scripts/dev-start-mac.sh` 一键脚本。该脚本仅启动前后端，不依赖 Docker，适合页面美化验证、接口联调等场景。

### 6.1 前置依赖

| 组件 | 版本要求 | 说明 |
|------|---------|------|
| JDK | 17 | 需要配置 `JAVA_HOME` 或已在 PATH 中 |
| Maven | 3.8.8+ | 需要已在 PATH 中 |
| Node.js | >= 16.14.0（推荐 v20+） | 需要已在 PATH 中 |
| pnpm | 8.8.0 | `npm install -g pnpm@8.8.0` |
| bash / curl / openssl | 任意 | 用于执行证书生成脚本 |
...
1. 环境检查
2. 生成证书（`bash scripts/test/setup.sh`）
3. 编译后端 fat jar（`mvn clean install -Dmaven.test.skip=true`）
4. 启动后端（HTTP 8080 / HTTPS 8443）
5. 启动前端开发服务器（`http://localhost:8000`）
...
这是预期行为。如需完整功能验证，请参考 [Ubuntu / Linux 一键开发环境](#2-一键启动本地开发环境)部署 Kuscia。
