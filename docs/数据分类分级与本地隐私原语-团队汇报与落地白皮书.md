# 数据分类分级与本地隐私原语 —— 团队汇报与落地白皮书

> 面向对象：研发团队、产品、安全合规、项目管理  
> 文档定位：比立项报告更复杂，用于团队内部对齐、技术汇报与后续开发落地  
> 涵盖范围：数据分类分级（Data Classification）+ 本地隐私保护原语（DP / K-匿名 / 脱敏 / 查询混淆）  
> 关联文档：
...
2. **本地隐私保护原语（Local Privacy Primitives）**：在数据被使用或出域前，自动或手动地执行脱敏、K-匿名、差分隐私、查询混淆四种保护手段。
...
我们要让平台具备“**看见敏感数据 + 自动保护敏感数据**”的闭环能力：
...
分类分级（自动识别 L1~L5）
   ↓
策略匹配（L5→TEE, L4→MPC/FL+DP, L3→DP/K-匿名/脱敏, L2→受限共享, L1→公开）
   ↓
隐私原语执行（批量组件 / 本地 SDK / REST / gRPC）
   ↓
审计日志 + 隐私预算台账
```
...
### 2.3 两者的关系：分类分级是“眼睛”，隐私原语是“手”

```
分类分级负责回答：这批数据有多敏感？
隐私原语负责回答：针对这个敏感等级，我们应该怎么处理？
```

例如：
- 分类分级发现 `brca1_status=L5`；
- 策略路由判定：L5 禁止明文出域，强制路由至 TEE；
- 隐私原语层：TEE 内执行安全计算，输出侧由脱敏组件处理。
...
分类分级和本地隐私原语解决的是“单点数据如何被识别和保护”。但在真实业务中，数据价值往往来自**跨机构、跨域的协同计算**：
...
本项目不是单纯增加几个算法，而是构建一套“**识别 → 决策 → 执行 → 审计**”的隐私保护闭环。分类分级让平台“看得见”敏感数据，本地隐私原语让平台“管得住”敏感数据。双模执行、参数可治理、多语言多格式，是这套能力能够从实验室走向生产环境、从平台侧走向业务侧的关键设计。
