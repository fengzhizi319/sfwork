...
# version_ge
#   功能：比较两个版本号，判断 $1 >= $2。
#   实现：用 sort -V 进行语义化排序，避免依赖 dpkg 这类 Debian/Ubuntu 工具。
version_ge() {
    printf '%s\n%s\n' "$2" "$1" | sort -V -C
}
...
#   参数：$1 - 端口号
#   返回：0=已被占用，1=空闲
#   说明：优先使用 ss，其次 lsof，最后回退到 netstat，尽量兼容 Linux/WSL。
port_in_use() {
    local port="$1"
    if command -v ss >/dev/null 2>&1; then
        ...
        netstat -an 2>/dev/null | grep -qiE "[:.]$port[[:space:]].*(LISTEN|LISTENING)"
    else
        return 1
    fi
}
...
check_dependencies() {
    ...
    # 使用通用的 sort -V 版本比较，避免依赖 dpkg。
    if [[ -n "$node_version" ]] && ! version_ge "$node_version" "16.14.0"; then
